...
3. Add `extensions/{id}.json`.
4. Add the extension to `registry.json`.
5. Open a PR.
...
- `registry.json` only points to per-extension metadata.
- `assetURL` must point to a GitHub Release asset.
- `sha256` must match the release asset.
- `manifest.id` must match the registry id.
- `manifest.version` must match `latestVersion`.
- `codexVersionRange` must match between the registry metadata and package manifest.
